...
--
DELIMITER $$
CREATE TRIGGER ActualizarStockLibrosDespuesDeInsertarPedido
...
BEGIN
    ...
    DECLARE cur CURSOR FOR 
        SELECT ID_Libro, Cantidad_libros
        FROM STOCK_LIBROS
        WHERE ID_Libro IN (
            SELECT ID_Libro
            FROM PEDIDOS
    ...
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO libro_id, cantidad;
        IF done THEN
            LEAVE read_loop;
        END IF;

        -- Disminuir el stock en STOCK_LIBROS por la cantidad vendida
        UPDATE STOCK_LIBROS
        SET Cantidad_libros = Cantidad_libros - cantidad
        WHERE ID_Libro = libro_id
        AND Cantidad_libros >= cantidad; -- Solo disminuir si hay suficiente stock
    END LOOP;
